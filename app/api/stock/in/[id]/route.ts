...
async function getRecord(
  ...
  return db.collection("stock_in").findOne({
    _id: new ObjectId(id),
    businessId: userId,
  });
}
...
export async function PATCH(
  ...
  try {
    ...
    if (countDiff < 0 && currentStockCount < Math.abs(countDiff)) {
      return NextResponse.json(
        { error: `Only ${currentStockCount} in stock. Cannot reduce in record by ${Math.abs(countDiff)}.` },
        { status: 400 }
      );
    }
    ...
    await db.collection("stock_in").updateOne(
    ...
    const updated = await db.collection("stock_in").findOne({
      _id: new ObjectId(id),
      businessId: userId,
    });
    ...
    console.error("Stock in PATCH error:", error);
...
export async function DELETE(
  ...
  try {
    ...
    const inCount = record.count ?? 0;
    const currentStockCount = stockItem.count ?? 0;
    if (currentStockCount < inCount) {
      return NextResponse.json(
        { error: `Cannot undo: only ${currentStockCount} left in stock.` },
        { status: 400 }
      );
    }
    ...
    await db.collection("stock_in").deleteOne({
      _id: new ObjectId(id),
      businessId: userId,
    });
    ...
    await db.collection("stock").updateOne(
      { _id: stockItem._id, businessId: userId },
      { $set: { count: currentStockCount - inCount, updatedAt: now } }
    );
    ...
    await db.collection("stock_history").insertOne({
      ...
      businessId: userId,
      previousCount: currentStockCount,
      newCount: currentStockCount - inCount,
      difference: -inCount,
      checkDate: now,
      note: "Stock in deleted - reversed",
      createdAt: now,
    });

    return NextResponse.json({ ok: true });
  } catch (error) {
    console.error("Stock in DELETE error:", error);
